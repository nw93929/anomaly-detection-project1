...
async def handle_sns(request: Request, background_tasks: BackgroundTasks):
    try:
        ...
        if msg_type == "Notification":
            ...
            for record in s3_event.get("Records", []):
                ...
                if key.startswith("raw/") and key.endswith(".csv"):
                    logger.info(f"Dispatching processing for: {key}")
        ...
        logger.error(f"Error handling SNS notification: {e}", exc_info=True)
...
def get_anomaly_summary():
    ...
    try:
        ...
        logger.error(f"Error fetching anomaly summary: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
